...
public class RealtimeNode extends BaseServerNode<RealtimeNode> {
    ...
    public RealtimeNode(Properties props,
                        Lifecycle lifecycle,
    ...
    public RealtimeNode setView(ServerView view) {
        Preconditions.checkState(this.view == null,
                                 "Cannot set view once it has already been set.");
        this.view = view;
        return this;
    }
    
    public RealtimeNode setMetadataUpdater(MetadataUpdater metadataUpdater) {
        Preconditions.checkState(this.metadataUpdater == null,
                                 "Cannot set metadataUpdater once it has already been set.");
        this.metadataUpdater = metadataUpdater;
        return this;
    }
    
    public RealtimeNode setSegmentPusher(SegmentPusher segmentPusher) {
        Preconditions.checkState(this.segmentPusher == null,
                                 "Cannot set segmentPusher once it has already been set.");
        this.segmentPusher = segmentPusher;
        return this;
    }
    
    public RealtimeNode setFireDepartments(List<FireDepartment> fireDepartments) {
        Preconditions.checkState(this.fireDepartments == null,
                                 "Cannot set fireDepartments once it has already been set.");
        this.fireDepartments = fireDepartments;
        return this;
    }
    
    public RealtimeNode registerJacksonInjectable(String name,
                                                  Object object) {
        Preconditions.checkState(injectablesMap.containsKey(name),
                                 "Already registered jackson object[%s]",
                                 name);
        injectablesMap.put(name,
                           object);
        return this;
    }
    ...
    protected void doInit() throws Exception {
        ...
        final RealtimeManager realtimeManager = new RealtimeManager(fireDepartments,
                                                                    conglomerate);
        lifecycle.addManagedInstance(realtimeManager);
        
        startMonitoring(monitors);
        
        final Context v2Druid = new Context(getServer(),
                                            "/druid/v2",
                                            Context.SESSIONS);
        v2Druid.addServlet(new ServletHolder(new StatusServlet()),
                           "/status");
        v2Druid.addServlet(new ServletHolder(new QueryServlet(getJsonMapper(),
                                                              getSmileMapper(),
                                                              realtimeManager,
                                                              emitter,
                                                              getRequestLogger())),
    ...
    protected void initializeJacksonInjectables() {
        ...
        for (Map.Entry<String, Object> entry : injectablesMap.entrySet()) {
            injectables.put(entry.getKey(),
                            entry.getValue());
        }
        
        injectables.put("queryRunnerFactoryConglomerate",
                        getConglomerate());
        injectables.put("segmentPusher",
                        segmentPusher);
        injectables.put("metadataUpdater",
                        metadataUpdater);
        injectables.put("serverView",
                        view);
        injectables.put("serviceEmitter",
                        getEmitter());
    ...
    private void initializeFireDepartments() {
        if (fireDepartments == null) {
            try {
                fireDepartments = getJsonMapper().readValue(new File(PropUtils.getProperty(getProps(),
                                                                                           "druid.realtime.specFile")),
    ...
    private void initializeSegmentPusher() {
        if (segmentPusher == null) {
            ...
            try {
                s3Client = new RestS3Service(new AWSCredentials(PropUtils.getProperty(props,
                                                                                      "com.metamx.aws.accessKey"),
                                                                PropUtils.getProperty(props,
                                                                                      "com.metamx.aws.secretKey")));
            } catch (S3ServiceException e) {
                throw Throwables.propagate(e);
            }
            
            segmentPusher = new S3SegmentPusher(s3Client,
                                                getConfigFactory().build(S3SegmentPusherConfig.class),
                                                getJsonMapper());
        }
    }
    ...
    protected void initializeMetadataUpdater() {
        if (metadataUpdater == null) {
            metadataUpdater = new MetadataUpdater(getJsonMapper(),
    ...
    private void initializeView() {
        if (view == null) {
            ...
            final ClientInventoryManager clientInventoryManager = new ClientInventoryManager(getConfigFactory().build(ClientConfig.class),
                                                                                             getPhoneBook(),
                                                                                             view);
    ...
    public static class Builder {
        ...
        public Builder withMappers(ObjectMapper jsonMapper,
                                   ObjectMapper smileMapper) {
        ...
        public RealtimeNode build() {
            if (jsonMapper == null && smileMapper == null) {
                ...
                smileMapper.getJsonFactory()
                           .setCodec(smileMapper);
