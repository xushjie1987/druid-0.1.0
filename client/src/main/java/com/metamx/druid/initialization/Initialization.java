...
public class Initialization {
    ...
    public final static String[]    SUB_PATH_PROPS = { "druid.zk.paths.announcementsPath", "druid.zk.paths.servedSegmentsPath", "druid.zk.paths.loadQueuePath", "druid.zk.paths.masterPath" };
    public static final String      DEFAULT_ZPATH  = "/druid";
    
    public static ZkClient makeZkClient(ZkClientConfig config,
                                        Lifecycle lifecycle) {
        final ZkClient retVal = new ZkClient(new ZkConnection(config.getZkHosts()),
    ...
    public static ZKPhoneBook createPhoneBook(ObjectMapper jsonMapper,
                                              ZkClient zkClient,
                                              String threadNameFormat,
                                              Lifecycle lifecycle) {
        return lifecycle.addManagedInstance(new ZKPhoneBook(jsonMapper,
                                                            zkClient,
    ...
    /**
     * Load properties. Properties are layered, high to low precedence: cmdLine -D,
     * runtime.properties file, stored in zookeeper. Idempotent. Thread-safe. Properties are only
     * loaded once. If property druid.zk.service.host=none then do not load properties from
     * zookeeper.
    ...
    public synchronized static Properties loadProperties() {
        ...
        Properties tmp_props = new Properties(fileProps); // the head of the 3 level Properties
                                                          // chain
        tmp_props.putAll(sp);
        ...
        for (String prop : fileProps.stringPropertyNames()) {
            log.info("Loaded(runtime.properties) Property[%s] as [%s]",
                     prop,
                     fileProps.getProperty(prop));
        }
        ...
        if (zk_hosts != null) {
            if (!zk_hosts.equals("none")) { // get props from zk
                ...
                zkPropLoadingClient = new ZkClient(new ZkConnection(clientConfig.getZkHosts()),
                ...
                if (propertiesZNodePath == null) {
                    String zpathBase = tmp_props.getProperty("druid.zk.paths.base",
                                                             DEFAULT_ZPATH);
                ...
                if (zkPropLoadingClient.exists(propertiesZNodePath)) {
                    Properties p = zkPropLoadingClient.readData(propertiesZNodePath,
                                                                true);
                    if (p != null) {
                        zkProps.putAll(p);
                    }
                ...
                for (String prop : zkProps.stringPropertyNames()) {
                    log.info("Loaded(properties stored in zk) Property[%s] as [%s]",
                             prop,
                             zkProps.getProperty(prop));
                }
            } // get props from zk
    ...
    public static CuratorFramework makeCuratorFrameworkClient(CuratorConfig curatorConfig,
    ...
    public static ServiceDiscovery makeServiceDiscoveryClient(CuratorFramework discoveryClient,
        ...
        final ServiceInstance serviceInstance = ServiceInstance.builder()
                                                               ...
                                                               .port(config.getPort())
                                                               .build();
        final ServiceDiscovery serviceDiscovery = ServiceDiscoveryBuilder.builder(Void.class)
    ...
    public static ServiceProvider makeServiceProvider(String serviceName,
    ...
    public static RequestLogger makeRequestLogger(ScheduledExecutorFactory factory,
                                                  Properties props) throws IOException {
        return new FileRequestLogger(factory.create(1,
                                                    "RequestLogger-%s"),
                                     new File(PropUtils.getProperty(props,
    ...
    /**
     * Validate and Resolve Properties. Resolve zpaths with props like druid.zk.paths.*Path using
     * druid.zk.paths.base value. Check validity so that if druid.zk.paths.*Path props are set, all
     * are set, if none set, then construct defaults relative to druid.zk.paths.base and add these
    ...
    public static boolean validateResolveProps(Properties props) {
        ...
        zpathValidateFailed = zpathBaseCheck(propertyZpath,
                                             "property druid.zk.paths.base");
        ...
        for (int i = 0; i < SUB_PATH_PROPS.length; i++) {
            ...
            if (val != null) {
                zpathOverrideCount++;
                if (!val.startsWith("/")) {
                    sbErrors.append(SUB_PATH_PROPS[i])
                            .append("=")
                            .append(val)
                            .append("\n");
        ...
        // separately check druid.zk.paths.propertiesPath (not in SUB_PATH_PROPS since it is not a
        // "dir")
        if (propertiesZpathOverride != null) {
            zpathOverrideCount++;
            if (!propertiesZpathOverride.startsWith("/")) {
                sbErrors.append("druid.zk.paths.propertiesPath")
                        .append("=")
                        .append(propertiesZpathOverride)
                        .append("\n");
                zpathValidateFailed = true;
            }
        }
        if (zpathOverrideCount == 0) {
            if (propertyZpath == null) { // if default base is used, store it as documentation
                props.setProperty("druid.zk.paths.base",
                                  zpathEffective);
            }
            //
            ...
            //
            for (int i = 0; i < SUB_PATH_PROPS.length; i++) {
                props.setProperty(SUB_PATH_PROPS[i],
                                  zpathEffective + "/" + SUB_PATHS[i]);
            }
            props.setProperty("druid.zk.paths.propertiesPath",
                              zpathEffective + "/properties");
        }
     ...
     * @param zpathBase
     *            znode base path, if null then this method does nothing.
     * @param errorMsgPrefix
     *            error context to use if errors are emitted, should indicate where the zpathBase
     *            value came from.
     * @return true if validate failed.
     */
    public static boolean zpathBaseCheck(String zpathBase,
                                         String errorMsgPrefix) {
        boolean zpathValidateFailed = false;
        if (zpathBase != null) {
            if (!zpathBase.startsWith("/")) {
                zpathValidateFailed = true;
                System.err.println(errorMsgPrefix +
                                   " must start with '/' (slash); found=" +
                                   zpathBase);
            }
            if (zpathBase.endsWith("/")) {
                zpathValidateFailed = true;
                System.err.println(errorMsgPrefix +
                                   " must NOT end with '/' (slash); found=" +
                                   zpathBase);
            }
        }
        return zpathValidateFailed;
