...
public class GroupByQueryEngine {
    ...
    public Sequence<Row> process(final GroupByQuery query,
                                 StorageAdapter storageAdapter) {
        final List<Interval> intervals = query.getQuerySegmentSpec()
                                              .getIntervals();
        if (intervals.size() != 1) {
            throw new IAE("Should only have one interval, got[%s]",
                          intervals);
        }
        
        final Iterable<Cursor> cursors = storageAdapter.makeCursors(Filters.convertDimensionFilters(query.getDimFilter()),
                                                                    intervals.get(0),
    ...
    private static class RowUpdater {
        ...
        public RowUpdater(ByteBuffer metricValues,
        ...
        private List<ByteBuffer> updateValues(ByteBuffer key,
            ...
            if (dims.size() > 0) {
                ...
                for (Integer dimValue : dims.get(0)
                                            .getRow()) {
                    ByteBuffer newKey = key.duplicate();
                    newKey.putInt(dimValue);
                    final List<ByteBuffer> unaggregatedBuffers = updateValues(newKey,
                                                                              dims.subList(1,
                                                                                           dims.size()));
                ...
                if (position == null) {
                    ...
                    positions.put(keyCopy,
                                  position);
                    thePosition = position;
                    for (int i = 0; i < aggregators.length; ++i) {
                        aggregators[i].init(metricValues,
                                            thePosition);
                ...
                thePosition = position;
                for (int i = 0; i < aggregators.length; ++i) {
                    aggregators[i].aggregate(metricValues,
                                             thePosition);
                    thePosition += increments[i];
                }
                return null;
    ...
    private class PositionMaintainer {
        ...
        public PositionMaintainer(int start,
                                  int[] increments,
                                  int max) {
            this.nextVal = start;
    ...
    private class RowIterator implements Iterator<Row> {
        ...
        public RowIterator(GroupByQuery query,
                           Cursor cursor,
        ...
        public Row next() {
            ...
            final PositionMaintainer positionMaintainer = new PositionMaintainer(0,
                                                                                 sizesRequired,
                                                                                 metricsBuffer.limit());
            final RowUpdater rowUpdater = new RowUpdater(metricsBuffer,
                                                         aggregators,
                                                         positionMaintainer);
            ...
            while (!cursor.isDone()) {
                ByteBuffer key = ByteBuffer.allocate(dimensions.size() *
                                                     Ints.BYTES);
                
                unprocessedKeys = rowUpdater.updateValues(key,
                                                          dimensions);
                if (unprocessedKeys != null ||
                    rowUpdater.getNumRows() > config.getMaxIntermediateRows()) {
                    break;
                }
            ...
            delegate = FunctionalIterator.create(rowUpdater.getPositions()
                                                           .entrySet()
                                                           .iterator())
                                         .transform(new Function<Map.Entry<ByteBuffer, Integer>, Row>() {
                                             private final long  timestamp  = cursor.getTime()
                                                                                    .getMillis();
                                             ...
                                             public Row apply(@Nullable Map.Entry<ByteBuffer, Integer> input) {
                                                 ...
                                                 ByteBuffer keyBuffer = input.getKey()
                                                                             .duplicate();
                                                 for (int i = 0; i < dimensions.size(); ++i) {
                                                     theEvent.put(dimNames[i],
                                                                  dimensions.get(i)
                                                                            .lookupName(keyBuffer.getInt()));
                                                 }
                                                 
                                                 int position = input.getValue();
                                                 for (int i = 0; i < aggregators.length; ++i) {
                                                     theEvent.put(metricNames[i],
                                                                  aggregators[i].get(metricsBuffer,
                                                                                     position));
